...
- **Repository** (`lib/db/repository.ts`): `listMetricSnapshots(clientId, source, from, to)`, `listAccessibleClients()` (no args — it reads `clients` as the signed-in user, so RLS decides: admin → all active; client/staff → own `client_id` only; unprovisioned → none), and `getDashboardOverview(client, days, now)`.
...
- **Read path** (`app/dashboard/`): `page.tsx` is a server page rendering `<Suspense fallback={<DashboardSkeleton />}>` around `DashboardView`, so `/dashboard` prerenders as a static shell (`○`) and edge-caches — an earlier server-rendered `app/page.tsx` that awaited `searchParams` was replaced by this (it made every client/range switch a full SSR, 1.5–2.9s on Vercel). `dashboard-view.tsx` owns the data on the client: it reads `?client=&days=&tab=` through `useSearchParams`, boots once via `/api/dashboard/boot` (profile + accessible clients + the requested selection), and fetches `/api/metrics/[clientId]/overview` for later selections.
- **Client cache + navigation**: payloads memoise in a `ref` Map keyed `${clientId}:${days}` with `CACHE_TTL_MS = 300s` (matching the server's `unstable_cache revalidate: 300`; the overview route's own `s-maxage` is 60, so the CDN refetches sooner and the server answers from cache), the first payload for a client triggers background prefetches of the remaining ranges, and a 60s interval plus `visibilitychange`/`focus` re-fetch a still-open selection once its entry ages out. Client/range changes `router.replace` inside `startTransition`; a section change uses `history.replaceState` alone (see the Section Deep-Link increment). Both write through `withDashboardParams` so neither drops the other's params. The skeleton stays mounted while boot is in flight rather than being swapped for a blank page, and is scoped to this route (`app/loading.tsx` would leak into `/profile`).
...
## Section Deep-Link Increment (`?tab=`)
The active dashboard section lives in the URL next to `client` and `days`, so a panel is shareable and switching one costs nothing.

**Status: implemented** (129 unit tests, typecheck, lint, build pass; measured against a production build).

- **Shared types** (`types/dashboard.ts`): `DASHBOARD_TABS` (`overview | queries | ai_visibility`), `DashboardTab`, `DEFAULT_DASHBOARD_TAB`, `isDashboardTab` — same shape as `DASHBOARD_RANGES`. Replaces the local `TAB_ORDER` copy that lived in `Dashboard`.
- **Pure URL state** (`lib/dashboard/url-state.ts`): `readDashboardTab(search)` falls back to the default for an absent or unknown value; `withDashboardParams(search, updates)` merges into the existing search string. Every write to `/dashboard?...` goes through the merge helper — the URL carries three params from three call sites, and a bare template string silently drops the ones it did not set.
