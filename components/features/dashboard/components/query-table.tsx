...
import { KeywordRankChart } from "./keyword-rank-chart";
import type { KeywordRankSeries, KeywordRow } from "@/types/dashboard";
...
export function QueryTable({ keywords, history }: QueryTableProps) {
  ...
  const activeKeyword = keywords.find((item) => item.keyword === selectedKeyword) ?? null;
  const series = selectedKeyword
    ? history.find((item) => item.keyword === selectedKeyword) ?? null
    : null;
  ...
  return (
    <section className="bg-surface rounded-xl border border-default overflow-hidden">
      ...
      {activeKeyword && series ? (
        <KeywordRankChart
          keyword={activeKeyword.keyword}
          rank={activeKeyword.rank}
          change={activeKeyword.change}
        ...
        <table className="w-full text-left text-xs">
          ...
          <thead className="bg-surface border-b border-default text-text-muted font-medium font-mono text-[11px]">
            <tr>
              ...
              <th scope="col" className="py-3 px-3 sm:px-4"><span className="sr-only">Trend</span></th>
            </tr>
          </thead>
          <tbody className="divide-y divide-border-default">
              ...
              <tr key={item.keyword} className="hover:bg-secondary/30 transition-colors">
                <td className="py-3 px-4 sm:px-5 font-medium text-text-primary">
                  <span className="block min-w-40">
                    {history.some((entry) => entry.keyword === item.keyword) ? (
                      <button
                        type="button"
                        onClick={() =>
                          setSelectedKeyword(
                        ...
                        className={item.keyword === selectedKeyword ? "text-accent-primary underline underline-offset-2" : "hover:text-accent-primary hover:underline underline-offset-2 transition-colors"}
                      >
                        {item.keyword}
                      </button>
                    ) : (
                ...
                <td className="py-3 px-3 sm:px-4 font-mono text-text-muted">
                  {item.volume === null ? "—" : item.volume.toLocaleString("en-US")}
                </td>
                <td className="py-3 px-3 sm:px-4 font-mono font-medium">#{item.rank}</td>
                <td className="py-3 px-3 sm:px-4">
                  {item.change === null ? (
                    <span className="inline-flex items-center text-text-muted font-mono">—</span>
                  ) : item.change > 0 ? (
                ...
                <td className="py-3 px-3 sm:px-4 text-right">
                  {history.some((entry) => entry.keyword === item.keyword) ? (
                    <button
                      ...
                      className="inline-flex items-center gap-1 text-text-muted hover:text-accent-primary transition-colors text-[11px]"
                      aria-label={`View rank trend for ${item.keyword}`}
                    >
                      <TrendingUp className="size-3" aria-hidden="true" /> Trend
                    </button>
                  ) : null}
                </td>
              </tr>
            ...
            {filteredKeywords.length === 0 && (
              <tr>
                <td colSpan={6} className="p-5 text-center text-text-muted">No keywords match your search.</td>
              </tr>
            )}
          </tbody>
        </table>
